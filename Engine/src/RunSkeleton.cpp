...
namespace Axiom
{
    ...
    RunSkeleton::RunSkeleton(Bone* bone, int _numBones)
        : mFirstBone(nullptr),
          mNumBones(_numBones)
    {
    ...
    RunSkeleton::~RunSkeleton()
    {
    }

    GameObjectAnim* RunSkeleton::GetFirstBone()
    {
        assert(mFirstBone);
        return mFirstBone;
    }

    GameObjectAnim* RunSkeleton::privFindBoneByIndex(int index)
    {
    ...
    void RunSkeleton::privSetAnimationHierarchy(Bone* pBoneResult)
    {
#if 1
        assert(pBoneResult);
        ...
        pPivot->SetTrans(0.0f, -8.0f, 3.2f);
