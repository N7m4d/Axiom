...
namespace Axiom
{
	HitLeftAnim::HitLeftAnim()
		: poSkeleton(nullptr)
	{
		this->poSkeleton = new HitLeftSkeleton(this->mBone, mNumBones);
		assert(this->poSkeleton);

		// Get this from the AnimMan
		this->mClip = new HitLeftClip(mNumBones);
		assert(mClip);
	}
	...
	AnimTime HitLeftAnim::FindMaxTime()
	{
		assert(mClip);
		return this->mClip->GetTotalTime();
	}

	void HitLeftAnim::PoseSkeleton(GameObjectAnimSkeleton* node)
	{
		// safety
		assert(node);
