...
namespace Axiom
{
    ...
    WalkSkeleton::WalkSkeleton(Bone* bone, int numBones)
        : mFirstBone(nullptr),
          mNumBones(numBones)
    {
    ...
    WalkSkeleton::~WalkSkeleton()
    {
    }

    GameObjectAnim* WalkSkeleton::GetFirstBone()
    {
        assert(mFirstBone);
        return mFirstBone;
    }

    GameObjectAnim* WalkSkeleton::privFindBoneByIndex(int index)
    {
    ...
    void WalkSkeleton::privSetAnimationHierarchy(Bone* bone)
    {
        assert(bone);
        ...
        pPivot->SetScale(150, 150, 150);
        pPivot->SetTrans(-4, 0, 0);
