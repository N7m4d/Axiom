...
namespace Axiom
{
	...
	HitBackSkeleton::HitBackSkeleton(Bone* pBone, int _numBones)
		: pFirstBone(nullptr),
		numBones(_numBones)
	{
	...
	HitBackSkeleton::~HitBackSkeleton()
	{

	}

	GameObjectAnim* HitBackSkeleton::GetFirstBone()
	{
		assert(pFirstBone);
		return this->pFirstBone;
	}

	GameObjectAnim* HitBackSkeleton::privFindBoneByIndex(int index)
	{
	...
	void HitBackSkeleton::privSetAnimationHierarchy(Bone* pBoneResult)
	{
#if 1
		assert(pBoneResult);
